...
def enter_bootstrap_chroot_python(lfs_dir: str) -> bool:
    ...
    if (
        os.system(
            f"chroot {lfs_dir} /usr/bin/env -i HOME=/root PATH=/bin:/usr/bin:/sbin:/usr/sbin /usr/bin/python3 /{SCRIPTS_FOLDER}/todd_linux/chroot/bootstrap_chroot_python.py"
        )
        != 0
    ):
        return False
...
def enter_install_from_chroot(lfs_dir: str, verbose: bool, jobs: int) -> bool:
    ...
    if (
        os.system(
            f"chroot {lfs_dir} /usr/bin/env -i HOME=/root PATH=/bin:/usr/bin:/sbin:/usr/sbin /usr/bin/python3 /{SCRIPTS_FOLDER}/main.py / --chroot {'-v' if verbose else ''} -j {jobs}"
        )
        != 0
    ):
        return False
